...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const movieId = params.movie || params.m;
  ...
  if (movieId && typeof movieId === "string") {
    ...
    if (movie) {
      ...
      const movieUrl = `${siteUrl}/movie/${encodeURIComponent(movieId)}`;
...
export default async function Page({ searchParams }: Props) {
  const params = await searchParams;
  const movieId = typeof params.movie === "string" ? params.movie : (typeof params.m === "string" ? params.m : undefined);
  return <MovieMelaClient initialMovieId={movieId} />;
}
